...
import type { WebCanvasMediaLoader } from '../../types/emscripten';
...
export class Video {
  ...
  #loader: WebCanvasMediaLoader | null = null;
  ...
  public dispose(): void {
    ...
    this.#loader = null;
    this.#bufPtr = 0; // free in the video
  ...
  #push(frame: VideoFrame): void {
    ...
    getModule().HEAPU8.set(bytes, this.#bufPtr);
    if (!this.#loader!.updateFrame(this.#currentTime)) {
      handleError('Failed to push the video frame', 'push');
    }
  }

  #rebind(w: number, h: number, operation: string): void {
    this.#loader = getModule().WebCanvasMediaLoader.setup(this.#ptr, w, h, this.#duration);
    this.#bufPtr = this.#loader ? this.#loader.buffer() : 0;
    if (!this.#loader) handleError('Failed to bind the video frame buffer', operation);
  }
