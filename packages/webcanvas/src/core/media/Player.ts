...
import { AudioSink, onUserGesture } from './Audio';
...
class MediaPlayer implements WebMediaPlayer {
  ...
  #ack(): void {
    this.#worker?.postMessage({ type: 'ack', time: this.#time });
  }
...
export class FramePlayer implements WebMediaPlayer {
  static pending: Video | null = null;
  ...
  static bind(video: Video, load: (token: number, size: number) => boolean): boolean {
    const Module = getModule();
    const token = Module._malloc(1);

    FramePlayer.pending = video;
    try {
      return load(token, 1);
    } finally {
      FramePlayer.pending = null;
      Module._free(token);
    }
  }
...
export function initMedia(Module: ThorVGModule): void {
  Module.createMediaPlayer = (_loader: number, bytes: Uint8Array) =>
    FramePlayer.pending ? new FramePlayer(FramePlayer.pending) : new MediaPlayer(bytes);
}
