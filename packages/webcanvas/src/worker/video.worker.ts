...
const video = { items: 0, bytes: 0, pending: null as { size: number; grant: () => void } | null };
...
/** Always admit one frame, a single frame larger than the budget would deadlock otherwise */
function admits(size: number): boolean {
  return video.items === 0 || (video.items < MAX_FRAMES && video.bytes + size <= MAX_FRAME_BYTES);
}

function reserve(size: number): Promise<void> {
  if (admits(size)) {
    video.items++;
    video.bytes += size;
  ...
  return new Promise<void>((resolve) => {
    video.pending = { size, grant: resolve };
  });
}

function ack(size: number): void {
  video.items = Math.max(0, video.items - 1);
  video.bytes = Math.max(0, video.bytes - size);
  const pending = video.pending;
  if (pending && admits(pending.size)) {
    video.pending = null;
    video.items++;
    video.bytes += pending.size;
    pending.grant();
  }
}
