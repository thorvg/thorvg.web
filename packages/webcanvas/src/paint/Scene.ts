...
export class Scene extends Paint {
  ...
  public resetEffects(): this {
    const Module = getModule();
    const result = Module._tvg_scene_reset_effects(this.ptr);
  ...
  public gaussianBlur(sigma: number, direction: number = 0, border: number = 0, quality: number = 75): this {
    const Module = getModule();
    const result = Module._tvg_scene_push_effect_gaussian_blur(this.ptr, sigma, direction, border, quality);
  ...
  public dropShadow(r: number, g: number, b: number, a: number, angle: number, distance: number, sigma: number, quality: number = 60): this {
    const Module = getModule();
    const result = Module._tvg_scene_push_effect_drop_shadow(this.ptr, r, g, b, a, angle, distance, sigma, quality);
   ...
   * const scene = new TVG.Scene();
   * scene.add(shape1, shape2);
   * scene.fill(255, 0, 0, 128); // Fill entire scene with semi-transparent red
   * ```
   */
  public fillEffect(r: number, g: number, b: number, a: number): this {
    const Module = getModule();
    const result = Module._tvg_scene_push_effect_fill(this.ptr, r, g, b, a);
  ...
  public tint(blackR: number, blackG: number, blackB: number, whiteR: number, whiteG: number, whiteB: number, intensity: number): this {
    const Module = getModule();
    const result = Module._tvg_scene_push_effect_tint(this.ptr, blackR, blackG, blackB, whiteR, whiteG, whiteB, intensity);
  ...
  public tritone(shadowR: number, shadowG: number, shadowB: number, midtoneR: number, midtoneG: number, midtoneB: number, highlightR: number, highlightG: number, highlightB: number, blend: number): this {
    const Module = getModule();
    const result = Module._tvg_scene_push_effect_tritone(this.ptr, shadowR, shadowG, shadowB, midtoneR, midtoneG, midtoneB, highlightR, highlightG, highlightB, blend);
