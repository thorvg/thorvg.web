...
import type { StrokeCapType, StrokeJoinType, FillRuleType } from '../constants';
...
export class Shape extends Paint {
   ...
   *     .lineTo(160, 180)
   *     .close()
   *     .fillRule('evenodd')  // Use even-odd rule for star shape
   *     .fill(255, 200, 0, 255);
   * ```
   */
  public fillRule(rule: FillRuleType): this {
    const Module = getModule();
    const ruleMap: Record<FillRuleType, number> = {
      winding: FillRule.Winding,
      evenodd: FillRule.EvenOdd,
    };
    const result = Module._tvg_shape_set_fill_rule(this.ptr, ruleMap[rule]);
