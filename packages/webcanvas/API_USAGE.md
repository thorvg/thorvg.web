...
ThorVG WebCanvas provides an object-oriented API supporting WebGPU, WebGL, and Software rendering backends.
...
Adds a Paint object (Shape, Scene, Picture, Text) to the canvas.
...
**Parameters:**
- `paint: Paint` - Paint object to add
...
- `options.width?: number` - Width for raw image format
- `options.height?: number` - Height for raw image format
- `options.colorSpace?: ColorSpace` - Color space for raw image format
...
picture.load(new Uint8Array(arrayBuffer), { type: 'png' });

// Load from URL
const svgResponse = await fetch('/images/logo.svg');
...
TVG.Font.load('MyFont', new Uint8Array(fontData), { type: 'ttf' });
...
**Parameters:**
- `factor: number` - Uniform scale factor
...
ThorVG WebCanvas provides automatic memory management.
...
async function main() {
  ...
  canvas.add(scene).add(text).add(animation.picture);
