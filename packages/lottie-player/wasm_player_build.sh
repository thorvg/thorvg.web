...
if [[ "$BACKEND" == "wg" ]]; then
  ...
  sed "s|EMSDK:|$EMSDK|g" ../wasm/wasm32_wg.txt > /tmp/.wasm_cross.txt
  meson setup -Db_lto=true -Ddefault_library=static -Dstatic=true -Dloaders="lottie, png" -Dextra="lottie_exp" -Dthreads=false -Dpartial=false -Dengines="wg" -Dfile="false" --cross-file /tmp/.wasm_cross.txt build_wasm_player
else
