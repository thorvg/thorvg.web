...
const App = () => {
  ...
  useEffect(() => {
    const initWebCanvas = async () => {
      try {
        // Initialize ThorVG with WebGL renderer
        const TVG = await ThorVG.init({
          renderer: 'gl',
          locateFile: (path) => {
            // Extract filename and resolve to webcanvas dist
        ...
        // Draw blue circle with black stroke
        ...
        setError(err instanceof Error ? err.message : 'Failed to initialize WebCanvas');
  ...
  return (
    <div className="app">
      ...
      <section className="example-section">
        ...
        {initialized && <p className="success">WebCanvas initialized successfully!</p>}
        <canvas
          id="webcanvas"
          width={800}
          height={600}
