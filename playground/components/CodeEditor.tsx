...
declare module '@thorvg/webcanvas' {
  ...
  export interface Picture {
    loadData(data: string | Uint8Array, options: {
      format: string;
      resourcePath?: string;
      copy?: boolean;
      width?: number;
      height?: number;
      colorSpace?: 'argb8888' | 'abgr8888' | 'abgr8888s';
    }): Picture;
    size(width?: number, height?: number): { width: number; height: number } | Picture;
    translate(x: number, y: number): Picture;
    rotate(angle: number): Picture;
    scale(scale: number): Picture;
    opacity(opacity: number): Picture;
    clip(clipper: Shape): Picture;
  }
