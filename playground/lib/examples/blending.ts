...
let imageData;
(async () => {
  let response = await fetch('https://jinui.s3.ap-northeast-2.amazonaws.com/rawimage_200x300.raw');
  const arrayBuffer = await response.arrayBuffer();
  imageData = new Uint8Array(arrayBuffer);

  response = await fetch('/assets/images/tiger.svg');
  ...
  function blender(name, method, x, y) {
    ...
    //gradient blending
    {
      ...
      fill.addStop(0, [255, 0, 255, 255]);
      fill.addStop(1, [0, 255, 0, 127]);
      ...
      fill2.addStop(0, [255, 0, 255, 255]);
      fill2.addStop(1, [0, 255, 0, 127]);
    ...
    //image
    {
      ...
      bottom.loadData(imageData, { format: 'raw', width: 200, height: 300, colorSpace: TVG.ColorSpace.ARGB8888 });
    ...
    {
      const bottom = new TVG.Picture();
      bottom.loadData(svgData, { format: 'svg' });
