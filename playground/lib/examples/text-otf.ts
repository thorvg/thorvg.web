...
export const textOtfExample: ShowcaseExample = {
  id: 'text-otf',
  title: 'Text OTF',
  description: 'Render text with OpenType fonts',
  category: 'text',
  thumbnail: '/assets/text-otf-thumbnail.png',
  code: `// Native example: TextOtf.cpp
...
(async () => {
  ...
  await Promise.all(fonts.map(async (fontName) => {
    const response = await fetch(\`/fonts/\${fontName}.otf\`);
    const buffer = await response.arrayBuffer();
    TVG.Font.load(fontName, new Uint8Array(buffer), { type: 'otf' });
  }));

  const text1 = new TVG.Text();
  text1.font('DMSans')
    .fontSize(38)
    .text('THORVG OpenType Font')
    .fill(255, 255, 255);
  canvas.add(text1);
  ...
  text2.font('DMSans')
    .fontSize(18)
    .italic()
    .text('Font = "DMSans", Size = 18, Style = Italic')
    .fill(255, 255, 255)
    .translate(0, 88);
  canvas.add(text2);

  const text3 = new TVG.Text();
  text3.font('default')  //Use any font
    .fontSize(23)
  ...
  text7.font('Pretendard').fontSize(9)
  ...
  text8.font('Pretendard').fontSize(9)
  ...
  text9.font('Pretendard').fontSize(9)
