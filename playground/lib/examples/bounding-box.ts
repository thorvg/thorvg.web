...

{
  ...
  shape3.appendRect(150, -33.33, 50, 33.33, 13.33, 16.67);
...

{
  ...
  svg1.loadData(svgData1, { format: 'svg' })
    .size(60, 60)
...

{
  ...
  svg2.loadData(svgData2, { format: 'svg' })
    .size(40, 40)
...

{
  ...
  img1.loadData(imgSvg1, { format: 'svg' }).size(300, 300);
  scene1.add(img1);
  canvas.add(scene1);
...

{
  ...
  img2.loadData(imgSvg2, { format: 'svg' }).size(300, 300);
  scene2.add(img2);
  canvas.add(scene2);
...

{
  ...
  sceneShape2.close();
  sceneShape2.stroke({ width: 20, color: [255, 255, 255, 255], join: 'bevel' });
...

{
  ...
  sceneShape4.stroke({ width: 20, color: [0, 255, 255, 255], join: 'miter' });
...
allShapes.forEach(shape => {
  ...
  if (obb && obb.length === 4) {
    ...
    obbShape.moveTo(obb[0].x / dpr, obb[0].y / dpr);
    obbShape.lineTo(obb[1].x / dpr, obb[1].y / dpr);
    obbShape.lineTo(obb[2].x / dpr, obb[2].y / dpr);
    obbShape.lineTo(obb[3].x / dpr, obb[3].y / dpr);
    obbShape.close();
    obbShape.stroke({
      width: 1.33,
      color: [0, 0, 0, 255],
