...
(async () => {
  ...
  //Masking
  {
    ...
    mask.mask(nMask, TVG.MaskMethod.Alpha); // MaskMethod.Alpha
    shape.mask(mask, TVG.MaskMethod.Alpha); // MaskMethod.Alpha
    canvas.add(shape);

    //SVG
    const svg = new TVG.Picture();
    svg.loadData(svgData, { format: 'svg' });
    svg.opacity(0.392);
    svg.scale(1.125);
    ...
    svg.mask(mask2, TVG.MaskMethod.Alpha); // MaskMethod.Alpha
    canvas.add(svg);
    ...
    mask3.opacity(0.784);
    star.mask(mask3, TVG.MaskMethod.Alpha); // MaskMethod.Alpha
    canvas.add(star);

    const image = new TVG.Picture();
    image.loadData(imageData, { format: 'raw', width: 200, height: 300, colorSpace: TVG.ColorSpace.ARGB8888 }).size(75, 112.5);
    ...
    mask4.opacity(0.275);
    image.mask(mask4, TVG.MaskMethod.Alpha); // MaskMethod.Alpha
    canvas.add(image);
  }
  ...
  //Inverse Masking
  {
    ...
    mask.mask(nMask, TVG.MaskMethod.InvAlpha); // MaskMethod.InvAlpha
    ...
    mask2.fill(255, 255, 255);
    svg.mask(mask2, TVG.MaskMethod.InvAlpha); // MaskMethod.InvAlpha
    canvas.add(svg);
    ...
    mask3.fill(255, 255, 255);
    star.mask(mask3, TVG.MaskMethod.InvAlpha); // MaskMethod.InvAlpha
    canvas.add(star);

    const image = new TVG.Picture();
    image.loadData(imageData, { format: 'raw', width: 200, height: 300, colorSpace: TVG.ColorSpace.ABGR8888 }).size(75, 112.5);
    ...
    mask4.close();
    mask4.fill(255, 255, 255);
    mask4.opacity(70 / 255);
    image.mask(mask4, TVG.MaskMethod.InvAlpha); // MaskMethod.InvAlpha
    canvas.add(image);
  }
