...
sed "s|EMSDK:|$EMSDK/|g" ../wasm/wasm32.txt | \
  ...
  sed "s|'--bind'|'--bind', '--emit-tsd=thorvg.d.ts', '-sEXPORTED_FUNCTIONS=${EXPORTED_FUNCTIONS}', '-sEXPORTED_RUNTIME_METHODS=${EXPORTED_RUNTIME_METHODS}', '-sDISABLE_EXCEPTION_CATCHING=0', '-sDISABLE_EXCEPTION_THROWING=0'|g" > /tmp/.wasm_webcanvas_cross.txt
...
meson setup -Db_lto=true --cross-file /tmp/.wasm_webcanvas_cross.txt build_wasm_webcanvas ./wasm/webcanvas

if [ $? -ne 0 ]; then
  echo "ERROR: WebCanvas bindings meson setup failed!"
  exit 1
fi
...
if [ $? -ne 0 ]; then
  echo "ERROR: WebCanvas bindings build failed!"
  exit 1
fi

ls -lrt build_wasm_webcanvas/*.{js,wasm}